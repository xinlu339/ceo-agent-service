...
| Graphify | Prompt 允许 `graphify query/explain/path` | `graphify_read` 永久只读 adapter，只允许 query/explain/path，使用 `execFile` 而非 shell，隔离 Provider secrets | `tests/test_pi_extension.py`、`tests/test_pi_capabilities.py`、`tests/test_prompt.py` | 代码已支持；本机 `graphify` CLI 未安装，doctor 为 `missing_cli` |
...
| Lark 读取/普通写入 | Codex passthrough MCP/CLI | 官方 `lark-cli schema` 和 shortcut help 提供风险分类；read 与普通 write 可同时启用 | `tests/test_pi_extension.py`、`tests/test_agent_runner.py` | Schema adapter ready；Channel Gate 因本机未登录而 blocked |
...
| NvWa | 只用于离线 work-profile 复核 | 专用 Runner 只暴露 workspace reads + `write_work_profile`，只能原子替换固定画像文件 | `tests/test_nvwa_review.py` | 本机 NvWa Skill 未安装，`missing_config`；普通 Runtime 不受影响 |
...
| Audit Web / Feedback | 查看 attempt、session、trace、feedback | 新运行展示 Pi session；旧 Codex 只读；现有 8765 UI 不改入口 | `tests/test_audit_web.py`、feedback tests | 可用；用户手动进程需自行重启加载新代码 |
...
2. **实现完成但本机缺外部授权/安装**：Xiaoqing OAuth、Lark 登录、NvWa Skill、Graphify CLI。它们不能从支持范围删除，但也不能伪报真实调用已通过。
