...
3. 使用已注入材料、本地文件以及 reviewed DWS 只读工具的文档、知识库、AI听记、群聊、日志、待办、日历等能力寻找独立证据。Friday Memory reviewed read tools 配置可用时，可以用 memory_recall 补充历史背景；相关证据位于 Lark 时使用 reviewed Lark read，涉及公开资料时可使用 Exa，只在招聘或候选人 KR 确实需要时使用 Xiaoqing read。只能调用本轮实际暴露且已配置的 read tools；工具未配置或授权失败时继续使用其他材料并记录证据缺口。不得调用 memory_write、document_upload 或任何写工具。文档型产出必须找到并读取正文；只找到标题按未找到处理。无法独立访问的业务系统若进展给出明确数字，可作为工作事实，但要写清审计缺口。
